# Publications TMT

## Scientific Submission Documents

TMT is now **ready for submission** with complete documents for scientific publication.

## Available Materials

### [Zenodo Package](zenodo_package.md)
Complete set of data, scripts, and results for scientific archiving.

### [Scientific Article](scientific_article.md)
Complete manuscript for peer-reviewed journal submission.

## Publication Status
- ✅ **Validated data**: 100% SPARC, Hubble tension resolved
- ✅ **Reproducible scripts**: Complete quantitative tests
- ✅ **Complete documentation**: FR/EN/ES
- ✅ **Testable predictions**: Distinctive vs ΛCDM

## Submission Targets
- **Astrophysical Journal**: Observational cosmology
- **Monthly Notices RAS**: Galactic dynamics
- **Nature Astronomy**: Major alternative theory
- **Zenodo**: Permanent data archiving

---

*TMT: Validated theory ready for scientific publication*