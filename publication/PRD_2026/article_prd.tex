...
in which a single scalar field $\psi$, the \emph{temporon}, is
...
and the strict scalar nature of the temporon contribution to weak
...
the \emph{temporon}, non-minimally coupled to the Ricci scalar via
...
from the temporon stress-energy tensor;
...
temporon equations. Section~\ref{sec:galactic} presents the
...
and $\Lambda_{0}$ is a bare cosmological constant. The temporon
...
collecting the temporon-curvature contributions on the left-hand side,
\begin{equation}
\label{eq:einstein}
...
\emph{the strength of gravity depends locally on the temporon condensate}.

\subsection{Temporon field equation}
...
For static or slowly varying configurations the temporon equation
...
\end{equation}
above which the temporon condenses. With the SPARC-calibrated
...
becomes very large at $z\gtrsim 1$, forcing the temporon back to
...
disk galaxies, the leading-order temporon equation reduces to
\begin{equation}
\label{eq:psi_static}
...
the temporon condenses to $\psi\approx v$; outside the system
...
yields after integration of the temporon stress-energy contribution
\begin{equation}
\label{eq:Meff}
...
which the temporon model does not improve over Newton are baryon-dominated
systems whose rotation curves are well described by the
Newtonian baseline alone, consistent with the framework's
expectation that the temporon contribution becomes negligible
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{figure_rotation_curves_PRD.png}
...
to giant spirals (UGC2885). Solid black: temporon prediction
$V(r)=\sqrt{G\,\Meff(r)/r}$ from Eq.~(\ref{eq:Meff}). Gold dashed:
Newtonian baryonic prediction. Blue dotted: temporon contribution
...
\begin{table}[h]
\caption{\label{tab:sparc} Performance of the temporon scalar-tensor
...
\begin{split}
\log_{10}(\rc/\kpc) = {} & (0.48\pm 0.04)\,
   \log_{10}\!\bigl(\Mbary/10^{10}\Msun\bigr) \\
   {} & + \log_{10}(8.7),
\end{split}
\end{equation}
...
the chameleon-type screening~\cite{Khoury2004}: the temporon
...
paper. The temporon scalar-tensor theory is, to our knowledge,
...
\begin{table}[t]
\caption{\label{tab:comparison} Predictions of the temporon
scalar-tensor model relative to alternative frameworks
(``BD'': Brans--Dicke), restricted to the galactic and solar-system
regimes addressed in this paper.
...
Phenomenon & $\Lambda$CDM & MOND & $f(R)$ & BD & This work \\
\hline
Galactic rotation curves            & $\circ$      & $\checkmark$ & $\circ$ & $\circ$ & $\checkmark$ \\
$\rc\!\propto\!\Mbary^{1/2}$        & $\circ$      & $\times$     & $\circ$ & $\times$ & $\checkmark$ \\
Bullet Cluster / lensing            & $\checkmark$ & $\times$~\cite{Clowe2006} & $\circ$ & $\circ$ & ($\circ$, see~\ref{sec:limitations}) \\
Solar-system PPN                    & $\checkmark$ & $\times$     & $\circ$ & $\checkmark$ & $\checkmark$ \\
\end{tabular}
\end{ruledtabular}
\end{table}
...
(below the $0.1\%$ level). The temporon contribution to lensing is
...
relativity in which a single new degree of freedom -- the temporon
...
possible. This work received no external funding and was
conducted entirely independently.
\end{acknowledgments}

\section*{Data and Code Availability}

All analysis code, calibrated parameters and validation scripts
underlying the present manuscript are publicly available at
\url{https://github.com/chronos717313/Mastery-of-time} and archived
at Zenodo. The canonical version corresponding to this submission
is \href{https://doi.org/10.5281/zenodo.20114198}{10.5281/zenodo.20114198}
(versioned DOI), with the persistent concept DOI
\href{https://doi.org/10.5281/zenodo.18287042}{10.5281/zenodo.18287042}
pointing to the latest version. The SPARC catalogue is available
from~\cite{Lelli2016}.
