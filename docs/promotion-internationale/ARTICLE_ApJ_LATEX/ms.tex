...
%%
%% AAS SUBMISSION REQUIREMENTS MET:
%%   [x] Line numbers (\linenumbers)
%%   [x] Abstract < 250 words
%%   [x] \correspondingauthor
%%   [x] \facilities{}
%%   [x] \software{} in AASTeX format
%%   [x] Dataset DOIs acknowledged
%%   [x] \bibliographystyle before \bibliography
%%   [x] SNe~Ia nomenclature

\documentclass[manuscript,linenumbers]{aastex631}
%% NOTE: 'manuscript' = format recommandé ApJ pour soumission initiale
%%   - Colonne unique, double interligne (facile à annoter pour les reviewers)
%%   - Numéros de ligne automatiques (exigence AAS satisfaite)
%%   - Pas d'en-têtes courants (évite le conflit ORCID/date)
%%   - Équations et notation identiques à twocolumn

%% ---- packages -------------------------------------------------------
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{physics}       % for \bra, \ket, \braket
%% NOTE: hyperref is loaded automatically by aastex631 -- do not reload
...
%% Running headers — prevent ORCID from leaking into the page header
\shorttitle{Temporal Distortion: Dark Matter \& Dark Energy}
\shortauthors{Despr\'{e}s Asselin}
...
\subsection{\texorpdfstring{The $\rc(M)$ Law}{The rc(M) Law}}
\label{sec:rclaw}
...
\begin{itemize}
  \item $\beta_{\mathrm{SNe\,Ia}} = 0.001$: integrated effect along
...
\subsection{\texorpdfstring{The $k(M)$ Law}{The k(M) Law}}
\label{sec:kresult}
...
\subsection{\texorpdfstring{$\HObs$ Tension Resolution}{H0 Tension Resolution}}
\label{sec:h0}
...
\subsection{\texorpdfstring{Comparison with $\Lambda$CDM}{Comparison with LambdaCDM}}
\label{sec:comp_lcdm}

TMT uses 5 global parameters ($\rc$ slope and intercept, $n$,
$\beta_{\mathrm{SNe\,Ia}}$, $\beta_{\HObs}$) to describe phenomena that
...
\begin{enumerate}
  ...
  \item \textbf{Dual-$\beta$ structure.}  The two-regime $\beta$ model
    ($\beta_{\mathrm{SNe\,Ia}} = 0.001$, $\beta_{\HObs} = 0.82$) is
...
The author also acknowledges the KiDS collaboration, the COSMOS team,
and the Pantheon+ team for making their data publicly available.
This work received no external funding and was conducted entirely
independently.
\end{acknowledgments}

%% AAS REQUIREMENT: \facilities{} macro
\facilities{
  Spitzer (IRAC),
  \textit{Herschel},
  Planck,
...
%% AAS REQUIREMENT: \software{} macro (AASTeX native format)
\software{
  Python 3.11 \citep{python},
  NumPy \citep{Harris2020},
  SciPy \citep{Virtanen2020},
  Astropy \citep{Astropy2022}
}

%% AAS REQUIREMENT: Dataset acknowledgments
%% SPARC catalogue: http://astroweb.cwru.edu/SPARC/  (Lelli et al. 2016)
%% KiDS-450: http://kids.strw.leidenuniv.nl/  (Hildebrandt et al. 2017)
%% COSMOS2015: https://cosmos.astro.caltech.edu/  (Laigle et al. 2016)
%% Pantheon+: https://pantheonplussh0es.github.io/ (Scolnic et al. 2022)
%% Analysis code: https://github.com/chronos717313/Mastery-of-time
